...
// Post Card Component
const PostCard = ({ userInfo, data }: PostCardProps) => {
  // post loading state
  ...
  // all comments state
  const [allComments, setAllComments] = useState<Array<CommentState>>([]);

  // likes state
  const [likes, setLikes] = useState<Array<LikeState>>([]);
  ...
  useEffect(
    () =>
      onSnapshot(
        ...
        (snapshot) => {
          const temp: Array<CommentState> = [];
          snapshot.docs.forEach((document) => {
            temp.push({
              ...document.data(),
              id: document.id,
              comment: document.data()?.comment,
              commentedUsername: document.data()?.commentedUsername,
              commentedUserImage: document.data()?.commentedUserImage,
  ...
  useEffect(
    () =>
      onSnapshot(
        ...
        (snapshot) => {
          const temp: Array<LikeState> = [];
          snapshot.docs.forEach((document) => {
            temp.push({
              ...document.data(),
              id: document.id,
              likedPersonUsername: document.data()?.likedPersonUsername,
            });
          });
  ...
  return (
    <div className="flex flex-col bg-white my-7 border rounded-md shadow-sm">
      ...
      {/* caption and likes */}
      <p className="p-5 truncate flex flex-col">
        {likes.length > 0 && (
