...
const Modal = ({ open, closeModal, userInfo }: ModalProps) => {
  // file picker ref
  ...
  const handleCaptionInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.includes("@")) {
      ...
      const filteredSuggestions: Array<CommonState> =
        userInfo?.following.filter((person) =>
              ...
              <div>
                {selectedFile ? (
                  ...
                  <div
                    ...
                    onClick={() =>
                      filePickRef?.current?.dispatchEvent(
