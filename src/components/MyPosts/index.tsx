...
type MyPostsProps = {
  myPosts: Array<PostsState>;
};

const MyPosts = ({ myPosts }: MyPostsProps) => {
  // loading state for posts
  const [postLoading, setPostLoading] = useState<boolean>(true);
  // post load error state
  const [postLoadError, setPostLoadError] = useState<boolean>(false);
  ...
  return (
    <div className="flex flex-col w-1/2 mt-10 space-y-4">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
        {myPosts?.map((post) => (
          <div
            className="h-full w-full relative bg-white rounded-md px-4 py-3 shadow-md border"
            key={post.id}
          >
            <img
